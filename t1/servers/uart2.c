...
int transmit2 (char *q, int head, int tail) {
...
void uart2serv()
{
  ...
  RegisterAs("com2");
//  DPRINTOK ("Creating uart2 rx notifier.\n");
...
//    DPRINTOK("UART1: received from client tid %d, mesg of size %d, first char"
  //           " %c\r\n", client_tid, r, buf[0]);
    if (r < 0 || r > BUFSIZE)  {DPRINTERR ("bad reply\n"); PANIC; }
    switch (buf[0]) {
      ...
      case 'r': // from rxtd
        if (client_tid != rxtid) {
          DPRINTERR("UART2: WTF notifier didn't send us this RX, %d did\r\n",
                   client_tid);
          PANIC;
        }
        //DPRINTOK ("huh\n");
      ...
      //  DPRINTOK ("what the shit???\n");
        break;
      case 'g': // from getc client
       // bwprintf (COM2, "SOMEONE ASKED TO GET.\n");
        if (head == tail) { // circular buffer is empty
          ...
          if (r != 0) {
            DPRINTERR("UART2: reply failed with %d retval to client tid %d\r\n",
                      r, reader_queue[0]);
            PANIC;
          }
        }
      ...
      case 't': // from txtd
        ...
        cts = 1;
        if (ohead != otail) {
          ohead = transmit2 (output_queue, ohead, otail);
          cts = 0;
      ...
      case 'p': // from putc client
        ...
        *(uint*)(VIC2BASE+INTEN_OFFSET) = UART2_MASK; 
        ...
        break;
      default:
        DPRINTERR ("unknown message to uart2 server.\n");  
    }
  }
}
