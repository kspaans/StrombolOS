...
void enable_uart () {
  ...
  *(uint*)(UART2_BASE+UART_CTLR_OFFSET) = UARTEN_MASK | TIEN_MASK  | RIEN_MASK;
}
...
void enable_interrupts () {
  ...
  *(uint*)(VIC1BASE+INTEN_OFFSET)       = TC1OI_MASK | UART1RXINTR1_MASK |
                                          UART2RXINTR1_MASK; // ... enable the timer? what is difference
...
void bootstrap (struct td *tds, void (*f)(), int *stacks) {
  ...
  enable_uart ();
  enable_interrupts ();
  _kCreate(tds, USER_LOW, f, 0, 0, stacks);
